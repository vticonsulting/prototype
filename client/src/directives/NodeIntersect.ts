...
const setObserver = (
  el: HTMLBaseElementWithObserver,
...
const extractOptions = (options: unknown) => {
  ...
  if (options instanceof IntersectionObserver) {
    ...
    hasOwnProperty(options, 'observer') &&
    options.observer instanceof IntersectionObserver
  ) {
    observer = options.observer
    if (
      hasOwnProperty(options, 'onIntersect') &&
      options.onIntersect instanceof Function
    ) {
...
export default {
  ...
  update(
    el: HTMLBaseElementWithObserver,
    {value: newOptions}: DirectiveBinding,
  ) {
